...
```
kubectl create secret generic k8s-sam-way \
  --from-literal="mykey=mydata"
```

Print a hexdump of the `k8s-sam-way` secret stored in etcd:
...
gcloud compute ssh controller-0 \
  ...
  /registry/secrets/default/k8s-sam-way | hexdump -C"
```
...
```
gcloud compute firewall-rules create k8s-sam-way-allow-nginx-service \
  --allow=tcp:${NODE_PORT} \
  --network k8s-sam-way
```
